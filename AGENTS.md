...
| Launcher sources | The launcher has two single-line filter rows (horizontal scroll): a container/launch-source row above the app categories. Sources: the GUI container (`CONTAINER_ID`), `Flatpak` and other distroboxes registered by `hyprtomic-distrobox-apps`. There is no `All` chip (it only duplicated apps across sources) and no `Host` chip (the only host entry is the ghostty override; ghostty stays on `Super+Q`); `activeContainer` selects the first source until a chip is clicked. The exporter writes standard `~/.local/share/applications/<container>-*.desktop` files with `Exec=distrobox-host-exec distrobox-enter -n <c> -- …` (the GUI container has no `distrobox` CLI), tags them with `X-HyprTomic-Container=` and prunes exports of deleted containers. It scans the application dirs recursively (wine installs into `wine/Programs/...` subdirectories) and, when home is shared, only wraps entries whose `Exec` binary exists in that container, so one container never registers another container's or the host's apps. Only real distrobox commands count as "already exported" — container homes (`~/distrobox/<name>/`) are fine, so wine `WINEPREFIX` paths and apps installed there are registered. Entries whose id uwsm rejects (Steam shortcuts with spaces) launch through their `Exec`. The exporter runs as the `hyprtomic-distrobox-apps.service` user unit (`--watch`, polls the running-container list) plus a one-shot at session start. It also mirrors container-home icons (Steam `steam_icon_*`) into `~/.local/share/icons/` and writes Steam cover art to `~/.local/share/hyprtomic/app-icons/` for games whose clienticon is tiny or missing. |
...
- quickshell loads the shelf plus the launcher / control-center / notifications /
  ...
  filter chips are two single-line rows: container/launch-source (the GUI
  container, Flatpak, other distroboxes) above the app categories; both scroll
  horizontally when they overflow (`FilterChipRow.qml`). The container row has
  no `All` chip (it only duplicated apps across sources) and no `Host` chip
  (host entries are just the ghostty override; ghostty stays on `Super+Q`); the
  first source is selected by default. Exported entries whose id uwsm rejects
  (Steam shortcuts contain spaces) are launched through their `Exec` instead.
