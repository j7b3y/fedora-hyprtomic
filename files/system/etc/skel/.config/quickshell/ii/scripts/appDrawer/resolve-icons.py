...
Vendored from Base Dotfiles (quickshell/scripts/resolve-icons.py) for the
HyprTomic app drawer. Uses the same icon resolution algorithm as nwg-dock (a
GTK3 app), so the icon pack is consistent between the dock and the launcher.
...
def _init_theme() -> Gtk.IconTheme:
    ...
    if theme_name:
        ...
        # theme switches take effect on the very first lookup.
        Gtk.Settings.get_default().set_property("gtk-icon-theme-name", theme_name)
    theme = Gtk.IconTheme.get_default()
    # Flatpak exports live outside the container's own data dirs (the host's
    # /var/lib/flatpak is bind-mounted by distrobox), so add their icons to
    ...
    for extra in (
        ...
        if os.path.isdir(extra):
            theme.append_search_path(extra)
    return theme
...
def _manual_search(name: str) -> str | None:
    ...
    This catches icons in flatpak dirs that the default theme might not search
    if not registered.
    """
    if "/" in name:
