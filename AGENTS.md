...
| GTK / Qt theming | No catppuccin-gtk package: GTK colors come from `~/.config/gtk-{3,4}.0/gtk.css` (generated by `apply-theme.sh` on theme switch). Adwaita base + Papirus-Dark icons + Bibata-Modern-Classic cursor (baked into the dconf db). Qt uses qt6ct/kvantum with the catppuccin Kvantum theme from skel. |
| Python deps | The shell venv is built from `files/system/usr/share/hyprtomic/uv-requirements.txt` (currently empty; `resolve-icons.py` needs only the container's python-gobject + gtk3). |
...
- The Arch dotfiles are ported under `files/system/etc/skel/`: a Lua Hyprland
  ...
  hyprbind configs, nemo actions, the Bibata cursor theme and the wallpaper.
